...
from utils.const import debug_mode, pnp_input_size, img_input_size, gdr_mode, dtype, img_render_size, \
    vis_ratio_threshold, t_depth_min, t_depth_max, lm_cam_K
...
class RandomPoseRegularObjDataset(IterableDataset):
    def __init__(self, obj_list=None, transform=None, dtype=dtype, device=None, scene_mode=True, bg_img_path=None,
        ...
        self._get_model_meshes()

    def __iter__(self) -> Iterator:
        item = None
        while True:
            cam_K, obj_id, gt_cam_R_m2c, gt_cam_t_m2c = self._get_scene_gt_item(item)
            img = self._get_bg_img(item)
            yield self._get_sample(item, cam_K, obj_id, gt_cam_R_m2c, gt_cam_t_m2c, img)

    def _get_model_meshes(self):
    ...
    def _get_scene_gt_item(self, item):
        ...
        box2d_max = torch.linalg.solve(cam_K, torch.tensor([[img_render_size], [img_render_size], [1.]],
                                       dtype=self.dtype, device=self.device)).T  # [1, 3(XY1)], inv(K) @ [W, H, 1.].T
...
class RenderedPoseBOPObjDataset(RandomPoseBOPObjDataset):
    ...
    def _get_bg_img(self, item):
        img_path = os.path.join(self._data_path, '{:0>6d}/rgb/{:0>6d}.png'.format(
            2 if self._lmo_mode else self._scene_gt[item]['obj_id'][0], self._scene_id[item]))
        img = read_img_file(img_path, dtype=self.dtype, device=self.device)
        return img

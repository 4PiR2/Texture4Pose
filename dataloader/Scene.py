...
class Scene:
    def __init__(self, objects, cam_K, obj_id, gt_cam_R_m2c, gt_cam_t_m2c, width=512, height=512, device=None,
                 **kwargs):
        self.kwargs = kwargs
        self.objects = objects  # dict
        self.device = parse_device(device)
        self.cam_K = cam_K / cam_K[-1, -1]  # [3, 3]
        self.obj_id = obj_id  # [N]
    ...
    def _get_cameras(self):
        ...
        K[0, 2, 3] = K[0, 3, 2] = 1.
        return PerspectiveCameras(R=R, K=K, image_size=((self.height, self.width),), device=self.device, in_ndc=False)

    def _get_obj_meshes(self):
    ...
    @staticmethod
    def _get_func_get_texture_gt():
        def f(obj: ObjMesh):
            verts = obj.mesh.verts_packed()  # [V, 3(XYZ)]
            gt_texture = torch.cat([torch.full((len(verts), 1), obj.obj_id, device=obj.device), verts], dim=1)
            return TexturesVertex(gt_texture[None])  # [1, V, 4(IXYZ)]

        return f

    def _set_obj_textures(self, func_get_texture):
    ...
    def _get_rasterizer(self):
        rasterizer = MeshRasterizer(
            ...
            raster_settings=RasterizationSettings(
                ...
                blur_radius=0.,
                faces_per_pixel=1,
                max_faces_per_bin=sum([int(mesh.num_faces_per_mesh().sum()) for mesh in self.obj_meshes]) * 2,
                # bin_size=0,  # Noisy Renderings on LM: https://github.com/facebookresearch/pytorch3d/issues/867
            ),
        )
        return rasterizer

    def _get_gt_scene_meshes(self):
    ...
    def render_scene(self, f=None, ambient=((1.,) * 3,), diffuse=((0.,) * 3,), specular=((0.,) * 3,),
...
class SceneBatch(Scene):
    ...
    def _get_vis_ratio(self):
        return torch.ones(len(self.gt_cam_R_m2c), device=self.device)

...
class RotWithRegionHead(nn.Module):
    def __init__(self, in_channels, out_channels=4, num_layers=3, num_filters=256, kernel_size=3, output_kernel_size=1):
        ...
        self.conv_last = nn.Conv2d(num_filters, out_channels, kernel_size=output_kernel_size, padding=pad, bias=True)
        self.fc_last = nn.Linear(num_filters, 2, bias=True)
    ...
    def forward(self, x):
        x = self.features(x)
        return self.conv_last(x), self.fc_last(x.mean(dim=[-2, -1]))

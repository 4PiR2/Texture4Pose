...
def main():
    ...
    # mesh.textures = TexturesAtlas(torch.rand(1, len(faces), 1, 1, 3, device=faces.device))
    # mesh.textures = TexturesAtlas(torch.arange(len(faces), device=faces.device).reshape(1, -1, 1, 1, 1).expand(-1, -1, -1, -1, 3) % 2.)
